...
def grade_q1(uniqname):
	try:
		repo,path = clone('q1', uniqname, 'c4cs-f16-wk7')
	except sh.ErrorReturnCode as e:
		text = '''
...
'''.format('c4cs-f16-wk7', e.full_cmd, e.stdout.decode('utf8'), e.stderr.decode('utf8'))
		return 0, text
...
def grade_q2a(uniqname):
	try:
		repo,path = clone('q2a', uniqname, 'c4cs-f16-conflict1')
	except sh.ErrorReturnCode as e:
		text = '''
<p><strong>Error! Failed to clone {}</strong></p>
<p>Ran command: <tt>{}</tt></p>
...
'''.format('c4cs-f16-conflict1', e.full_cmd, e.stdout.decode('utf8'), e.stderr.decode('utf8'))
		return 0, text
...
def grade_q2b(uniqname):
	try:
		repo,path = clone('q2b', uniqname, 'c4cs-f16-conflict2')
	except sh.ErrorReturnCode as e:
		text = '''
<p><strong>Error! Failed to clone {}</strong></p>
<p>Ran command: <tt>{}</tt></p>
...
'''.format('c4cs-f16-conflict2', e.full_cmd, e.stdout.decode('utf8'), e.stderr.decode('utf8'))
		return 0, text
...
<p>This is the final test run before final grading Friday morning.</p>
<p><strong>NOTE:</strong> Repositories must be named exactly correctly!</p>
'''.format(name)
	for q, repo in (
			('1', 'c4cs-f16-wk7'),
			('2a', 'c4cs-f16-conflict1'),
			('2b', 'c4cs-f16-conflict2'),
			):
		try:
			email += '<hr /><h3>Question {} ({})</h3>'.format(q,repo)
			ref = clone('q' + q, uniqname, repo)
			try:
