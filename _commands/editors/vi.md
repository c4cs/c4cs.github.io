...
![toggle-case](../static/commands/img/toggle_case.gif)
...
![visual-block-copypaste](../static/commands/img/visual_block_copypaste.gif)
...
# Customizing a vim Installation
...
their personal vim configuration is just a matter of:
    ...
    git clone https://billy_magic@bitbucket.org/billy_magic/vimrc.git ~/vimrc
...
executable is the same as having the original vimscript source code. Because of
this, for simplicity's sake, developers of vim plugins usually just distribute
their plugins through GitHub. Installing these plugins is a matter of `git
clone`ing the plugins into the appropriate directories on your machine.
...
- Native support for multithreading ("asynchronous jobs"). Newer versions of vim
  does support this, but it didn't when neovim was first released. In the past,
...
- A terminal emulator. Yes, you can launch neovim from inside neovim.
...
plugins are available for vim (notably [vim-lldb](https://github.com/gilligan/vim-lldb),
...
from to vim on a machine that doesn't have neovim is usually just a matter of
...
Start with `vimtutor`. After installing vim (`sudo apt install vim`, on a
Debian/Ubuntu-based distro), just type that command into a terminal and hit
enter. It'll cover the basics better than we can do here.

Alternatively, go to the following link: [here.](http://www.openvim.com/)


## Things to Learn

Some neat features that vim offers without needing to install plugins:
...
* [Mapping Keys in Vim](http://vim.wikia.com/wiki/Mapping_keys_in_Vim_-_Tutorial_(Part_1))
  ...
  * Might I recommend `inoremap jk <ESC>` and `vnoremap jk <ESC>`? That lets you
...
* [Mr. Worldwide: The `g`lobal Command](http://vim.wikia.com/wiki/Power_of_g)
  * Let's say you have a Linux log file that's tens of thousands of lines long,
  ...
  `"NetworkManager"`. You could write a shell script to do that, or...
...
can configure this in your `.vimrc` or in `.lvimrc` files using localvimrc.
...
than deoplete, though I haven't personally checked.
