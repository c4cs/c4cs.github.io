...
\medskip
\noindent
\textit{So, why is it called \texttt{grep}?}
\href{https://en.wikipedia.org/wiki/Grep}{Wikipedia tells us} that
\texttt{grep}'s name, ``comes from the \texttt{ed} command g/re/p ({\bf g}lobally
search a {\bf re}gular expression and {\bf p}rint)''. Let's give that a go
eh?  Run \texttt{ed~cracklib-small} and try the command
\texttt{g/password/p}.
